...
//	private int threshold[] = new int[6];
	...
	public Direction move(int[][] board, int[] location, int[] sack, int[][] players, int[][] traders)
	{
		...
		int next_node = getBestPath(location, traders[op_trader]);
		next_location = shortest.getCoordinates(next_node);
		turn_number++;
		return getDirection(location[0],location[1],next_location[0],next_location[1]);
	}
	
	int getBestPath(int src_location[],int dest_location[])
	{
		...
		int next_node = 0;
		Vector<Integer> v = shortest.getShortestPath(src_location[0], src_location[1], dest_location[0], dest_location[1]);
		if(v == null || v.size() == 0)
			next_node = shortest.getMapping(dest_location[0], dest_location[1]);
		else
		{
